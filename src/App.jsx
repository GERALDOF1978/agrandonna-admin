        ...
        <header className="flex justify-between items-center mb-8 bg-white/50 backdrop-blur-sm p-4 rounded-3xl border border-white/50">
          ...
          {['sabores', 'bebidas', 'banners', 'equipe'].includes(aba) && (
            <button onClick={() => {
              ...
              setEdit(
                aba === 'sabores' ? { name: '', desc: '', prices: { grande: 0, gigante: 0, meio_metro: 0 }, img: '' } :
                aba === 'bebidas' ? { name: '', price: 0, img: '' } :
          ...
          <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
            <table className="w-full text-left">
              ...
              <tbody>{(aba === 'sabores' ? sabores : aba === 'bebidas' ? bebidas : aba === 'banners' ? banners : equipe).map(it => (
                <tr key={it.id} className="border-b border-gray-50 hover:bg-gray-50 transition-all group">
                  <td className="p-6 flex items-center gap-4">
                    {(it.img || it.imageUrl) ? (
                      <img src={it.img || it.imageUrl} className="w-14 h-14 rounded-2xl object-cover shadow-sm border-2 border-white"/>
                    ) : (
                      <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center text-gray-300 border-2 border-white">
                    ...
                    <div>
                      ...
                      {aba === 'sabores' && (
                        <p className="text-[11px] text-red-600 font-black italic mt-1 max-w-[350px] leading-tight uppercase">
                          {it.desc || '⚠️ Sem ingredientes cadastrados.'}
                        </p>
                      )}
                    </div>
                  </td>
                  <td className="p-6 font-black text-[10px] text-gray-500 uppercase">
                    {aba === 'bebidas' && it.price && <span className="text-green-600 font-bold bg-green-50 px-2 py-1 rounded">Preço: R$ {it.price.toFixed(2)}</span>}
                    {aba === 'sabores' && it.prices && (
                      <div className="flex flex-col gap-1">
                        <span className="bg-gray-100 px-2 py-0.5 rounded text-[9px] text-gray-600">G: R$ {it.prices.grande}</span>
                        <span className="bg-gray-100 px-2 py-0.5 rounded text-[9px] text-gray-600">GG: R$ {it.prices.gigante}</span>
                        {it.prices.meio_metro > 0 && <span className="bg-gray-100 px-2 py-0.5 rounded text-[9px] text-gray-600">1/2M: R$ {it.prices.meio_metro}</span>}
                      </div>
                    )}
                  ...
                  <td className="p-6 text-right space-x-2">
                    ...
                    <button onClick={async () => { if (window.confirm('Eliminar permanentemente?')) await deleteDoc(doc(db, col, it.id)) }} className="p-3 text-red-600 hover:bg-red-50 rounded-2xl transition-all"><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))}</tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex justify-center items-center p-4 z-[100]">
          <form onSubmit={salvar} className="bg-white rounded-[50px] w-full max-w-lg p-10 space-y-5 shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-4">
              ...
              {aba === 'sabores' && <textarea placeholder="Ingredientes da Pizza (Ex: Mussarela, tomate, manjericão...)" className="w-full h-32 p-5 bg-gray-50 border border-gray-100 rounded-3xl font-bold outline-none focus:border-red-500" value={edit.desc} onChange={e => setEdit({ ...edit, desc: e.target.value })} />}
              {aba === 'sabores' && (
                <div className="grid grid-cols-2 gap-4">
