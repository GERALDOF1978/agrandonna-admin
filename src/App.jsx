        ...
        <header className="flex justify-between items-center mb-8 bg-white/50 backdrop-blur-sm p-4 rounded-3xl border border-white/50">
          ...
          {['sabores', 'bebidas', 'banners', 'equipe'].includes(aba) && (
            <button onClick={() => {
              ...
              setEdit(
                aba === 'sabores' ? { name: '', desc: '', description: '', prices: { grande: 0, gigante: 0, meio_metro: 0 }, img: '' } :
              ...
              <tbody>{(aba === 'sabores' ? sabores : aba === 'bebidas' ? bebidas : aba === 'banners' ? banners : equipe).map(it => (
                <tr key={it.id} className="border-b border-gray-50 hover:bg-gray-50 transition-all group">
                  ...
                  <td className="p-6 text-right space-x-2">
                    ...
                    <button onClick={async () => { if (window.confirm('Eliminar permanentemente?')) await deleteDoc(doc(db, aba === 'sabores' ? 'menu_sabores' : aba === 'bebidas' ? 'menu_bebidas' : aba === 'banners' ? 'menu_banners' : 'admin_users', it.id)) }} className="p-3 text-red-600 hover:bg-red-50 rounded-2xl transition-all"><Trash2 size={16}/></button>
                  </td>
                </tr>
              ))}</tbody>
            </table>
