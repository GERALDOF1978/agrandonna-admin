  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans">
      ...
      <main className={`flex-1 p-4 md:p-10 overflow-y-auto transition-colors duration-300 ${aba==='pedidos'?'bg-gray-300':'bg-gray-50'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {pedidos.map(p => (
              <div key={p.id} className={`bg-white rounded-[40px] shadow-xl border-t-8 p-6 flex flex-col gap-4 ${p.status==='pendente'?'border-red-600 animate-pulse':'border-transparent'}`}>
                ...
                {/* BOTÕES DE STATUS COM CORES ESPECÍFICAS */}
        ...
        {/* LISTA DE ITENS - SABORES IGUAL BEBIDAS COM INGREDIENTES */}
                ...
                <tr key={it.id} className="border-b border-gray-50 hover:bg-gray-50 transition-all">
                  <td className="p-6 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {/* INGREDIENTES EM VERMELHO E NEGRITO LOGO ABAIXO DO TÍTULO */}
                      {aba === 'sabores' && (
                        <p className="text-[11px] text-red-600 font-bold italic mt-1 max-w-[350px] leading-tight uppercase">
          ...
          <div className="max-w-md bg-white p-10 rounded-[50px] shadow-2xl border space-y-6 mx-auto">
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="number" className="w-full p-4 bg-gray-50 border rounded-[24px] font-bold" value={cfg.tempo} onChange={e=>setCfg({...cfg, tempo: e.target.value})} placeholder="Tempo"/>
                  <input type="number" className="w-full p-4 bg-gray-50 border rounded-[24px] font-bold" value={cfg.taxa} onChange={e=>setCfg({...cfg, taxa: parseFloat(e.target.value)})} placeholder="Taxa R$"/>
                </div>
                <input className="w-full p-4 bg-gray-50 border rounded-[24px] font-bold" value={cfg.topo} onChange={e=>setCfg({...cfg, topo: e.target.value})} placeholder="Título Cupom"/>
             </div>
