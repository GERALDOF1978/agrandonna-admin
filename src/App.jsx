...
export default function App() {
  ...
  // CONFIGURAÇÕES ATUALIZADAS COM A LÓGICA DE FRETE DINÂMICO
  ...
  useEffect(() => {
    ...
    const unsubC = onSnapshot(doc(db, 'loja_config', 'geral'), s => {
      if(s.exists()){
        // Garante que se a config for antiga, ela receba os campos novos
        const data = s.data();
        setCfg({
          ...data,
      ...
      <div key={p.id} className={`bg-white rounded-[40px] shadow-2xl border-t-8 p-6 flex flex-col gap-4 relative overflow-hidden ${p.status === 'pendente' ? 'border-red-600' : 'border-transparent shadow-gray-200'}`}>
        ...
        <div className="flex justify-between border-b border-gray-50 pb-2 relative z-10">
          ...
          <div className="flex gap-2">
            ...
            {/* TRAVA DE IMPRESSÃO: Botão só aparece se o status passou de Pendente */}
            {p.status !== 'pendente' && (
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans">
      ...
      <main className={`flex-1 p-4 md:p-10 overflow-y-auto transition-colors duration-300 ${['pedidos','historico'].includes(aba) ? 'bg-gray-300' : 'bg-gray-50'}`}>
        ...
        {aba === 'sistema' && (
          <div className="max-w-xl bg-white p-10 rounded-[50px] shadow-2xl border border-gray-100 space-y-6 mx-auto">
             ...
             {/* NOVOS CAMPOS: CÁLCULO DE FRETE DINÂMICO */}
             <div className="space-y-4 pt-4 border-t border-gray-100">
             ...
             <button onClick={async () => { await setDoc(doc(db, 'loja_config', 'geral'), cfg); alert('Sistema Atualizado com Sucesso!'); }} className="w-full bg-black text-white py-6 rounded-[30px] font-black uppercase shadow-xl hover:scale-95 transition-all">Guardar Configurações</button>
          </div>
        )}
      </main>
