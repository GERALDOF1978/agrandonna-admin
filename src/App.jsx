...
export default function Admin() {
  ...
  const [bans, setBans] = useState([]); const [eqp, setEqp] = useState([]);
  const [edit, setEdit] = useState(null); const [upL, setUpL] = useState(false);
  const [mst, setMst] = useState(false); const [filtro, setFiltro] = useState(new Date().toISOString().split('T')[0]);
  ...
  useEffect(() => {
    ...
    onSnapshot(collection(db,'admin_users'), s => setEqp(s.docs.map(d=>({id:d.id,...d.data()}))));
  ...
  if (!prm) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <button onClick={()=>signInWithPopup(auth,provider)} className="bg-white text-black p-6 rounded-[32px] font-black uppercase flex items-center gap-3 shadow-2xl active:scale-95 transition-all">
        ...
        <nav className="space-y-1 flex-1">
          {['pedidos','sabores','bebidas','banners','caixa','equipe','sistema'].map(m => (
            <button key={m} onClick={()=>setAba(m)} className={`w-full p-4 rounded-2xl font-black text-[10px] uppercase flex items-center justify-between transition-all ${aba===m?'bg-red-600 shadow-xl scale-105':'text-gray-500 hover:bg-gray-900'}`}>
              ...
              {m==='pedidos' && pndt > 0 && <span className="bg-white text-red-600 px-2 py-0.5 rounded-full text-[10px] animate-bounce">{pndt}</span>}
            </button>
          ))}
        </nav>
        <button onClick={()=>signOut(auth)} className="text-gray-600 font-bold text-[10px] uppercase flex items-center gap-2 hover:text-red-500 p-2"><LogOut size={14}/> Sair do Sistema</button>
      </aside>
      ...
      <main className={`flex-1 p-4 md:p-10 overflow-y-auto ${aba==='pedidos'?'bg-gray-300':'bg-gray-50'}`}>
        <header className="flex justify-between items-center mb-8">
          ...
          {['sabores','bebidas','banners','equipe'].includes(aba) && <button onClick={()=>{
              ...
              setEdit(aba==='sabores'?{name:'',desc:'',prices:{grande:0,gigante:0},img:''}:aba==='bebidas'?{name:'',price:0,img:''}:aba==='equipe'?{nome:'',email:''}:{title:'',imageUrl:''});
          }} className="bg-green-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase shadow-lg shadow-green-900/20 active:scale-95 transition-all">Novo {aba}</button>}
        </header>

        {aba === 'pedidos' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {peds.map(p => (
              <div key={p.id} className={`bg-white rounded-[40px] shadow-xl border-t-8 p-6 flex flex-col gap-3 ${p.status==='pendente'?'border-red-600 animate-pulse':'border-gray-100'}`}>
                ...
                <div className="grid grid-cols-2 gap-1.5 mt-2">
                  {['pendente','preparando','saiu_entrega','entregue'].map(st=>(<button key={st} onClick={()=>updateDoc(doc(db,'pedidos',p.id),{status:st})} className={`p-2 rounded-xl text-[7px] font-black uppercase transition-all ${p.status===st?'bg-red-600 text-white shadow-md':'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>{st.replace('_',' ')}</button>))}
                </div>
        ...
        {aba === 'caixa' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-green-100"><p className="text-[10px] font-black text-gray-400 uppercase">Faturamento</p><p className="text-4xl font-black text-green-600">R$ {cx.t.toFixed(2)}</p></div>
              <div className="bg-white p-8 rounded-[40px] shadow-sm border"><p className="text-[10px] font-black text-gray-400 uppercase">Ticket Médio</p><p className="text-4xl font-black text-gray-800">R$ {cx.avg.toFixed(2)}</p></div>
              <div className="bg-white p-8 rounded-[40px] shadow-sm border"><p className="text-[10px] font-black text-gray-400 uppercase">Vendas OK</p><p className="text-4xl font-black text-blue-600">{cx.q}</p></div>
            </div>
            <div className="bg-white p-8 rounded-[40px] border shadow-sm space-y-4">
              ...
              {cx.i.map(([n, q]) => (<div key={n} className="flex justify-between font-bold text-sm border-b border-gray-50 pb-2"><span>{n}</span><span className="bg-red-50 text-red-600 px-4 py-1 rounded-full text-xs font-black">{q}x</span></div>))}
            </div>
          </div>
        )}
        ...
        {['sabores','bebidas','banners','equipe'].includes(aba) && (
          <div className="bg-white rounded-[40px] shadow-sm border overflow-hidden">
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b text-[10px] font-black text-gray-400 uppercase"><tr><th className="p-6">Item / Descrição</th><th className="p-6">Valores / Email</th><th className="p-6 text-right">Ações</th></tr></thead>
              <tbody>{(aba==='sabores'?sabs:aba==='bebidas'?bebs:aba==='banners'?bans:eqp).map(it => (
                <tr key={it.id} className="border-b border-gray-50 hover:bg-gray-50 transition-all">
                  <td className="p-6 flex items-center gap-4">
                    {(it.img || it.imageUrl) && <img src={it.img || it.imageUrl} className="w-16 h-16 rounded-2xl object-cover shadow-sm border-2 border-white"/>}
                    <div>
                      <p className="font-black uppercase text-xs text-gray-900 tracking-tighter">{it.name||it.title||it.nome}</p>
                      {aba==='sabores' && <p className="text-[10px] text-red-600 font-bold italic mt-1 max-w-[350px] leading-tight">{it.desc || '⚠️ Sem ingredientes'}</p>}
                    </div>
                  </td>
                  <td className="p-6 font-black text-[10px] text-gray-500 uppercase">
                    {it.price ? `R$ ${it.price.toFixed(2)}` : it.prices ? `G: ${it.prices.grande} | GG: ${it.prices.gigante}` : it.email || '-'}
                  </td>
        ...
        {aba === 'sistema' && (
          <div className="max-w-md bg-white p-10 rounded-[50px] shadow-2xl border space-y-6 mx-auto">
             <div className="flex flex-col items-center gap-3 p-4 bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-200">
                ...
                <label className="bg-black text-white px-4 py-2 rounded-xl text-[10px] font-black cursor-pointer hover:bg-red-600 transition-all uppercase"><Upload size={14}/>
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input type="number" className="p-4 bg-gray-50 border rounded-[24px] font-bold" value={cfg.tempo} onChange={e=>setCfg({...cfg, tempo: e.target.value})} placeholder="Minutos"/>
                  <input type="number" className="p-4 bg-gray-50 border rounded-[24px] font-bold" value={cfg.taxa} onChange={e=>setCfg({...cfg, taxa: parseFloat(e.target.value)})} placeholder="Taxa R$"/>
                </div>
             ...
             <button onClick={async ()=>{await setDoc(doc(db,'loja_config','geral'), cfg); alert('Salvo!')}} className="w-full bg-black text-white py-6 rounded-[30px] font-black uppercase shadow-2xl active:scale-95 transition-all">Salvar Tudo</button>
          </div>
        )}
      </main>
      ...
      {edit && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex justify-center items-center p-4 z-[100]">
          <form onSubmit={salvar} className="bg-white rounded-[50px] w-full max-w-lg p-10 space-y-5 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h2 className="text-2xl font-black uppercase italic border-b pb-4 flex justify-between items-center text-gray-900 tracking-tighter">Configurar {aba} <button type="button" onClick={()=>setEdit(null)}><X size={30} className="text-gray-300 hover:text-black"/></button></h2>
            {['sabores','bebidas','banners'].includes(aba) && (
              <div className="flex flex-col items-center gap-4 p-4 bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-200">
                 ...
                 <label className="bg-black text-white px-6 py-2 rounded-2xl text-[10px] font-black uppercase cursor-pointer hover:bg-red-600 transition-all flex gap-2 items-center">
                   {upL ? <BarChart3 className="animate-spin" size={14}/> : <Upload size={14}/>} {upL ? 'Subindo...' : 'Carregar Foto'}
            ...
            {aba==='equipe' && <input placeholder="Gmail do funcionário" className="w-full p-5 bg-gray-50 border rounded-3xl font-bold" value={edit.email} onChange={e=>setEdit({...edit, email: e.target.value})} />}
